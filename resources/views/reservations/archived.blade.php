...
@section('content')
    <div class="dashboard-header">
        <h1>
            <i class="fa fa-users" aria-hidden="true"></i> <a href="{{ route('index.reservations') }}">Reservation Enquiries</a> / Archived
            <span class="pull-right">
                <!-- Trigger the modal with a button -->
                <a href="{{ route('index.reservations') }}" class="btn orange-button btn-archived not-on-mobile">
                    View Unarchived
                </a>
            </span>
        </h1>

        <a href="{{ route('index.reservations') }}" class="btn orange-button btn-archived mobile-dashboard-button mobile-only">
            View Unarchived
        </a>
    </div>
            ...
            <div class="row dashboard-entry review">
                <div class="col-xs-12 col-sm-6 col-lg-8">
                    ...
                    @if($reservation->phone)
                        ...
                        <i class="fa fa-phone" aria-hidden="true"></i> <span style="margin-left:10px;">{{ $reservation->phone }}</span>
                    @endif
                ...
                <div class="col-xs-6 col-sm-3 col-lg-2">
                    <form class="" method="POST" action="{{ route('unarchive.reservation', $reservation->id) }}">
                        {{ csrf_field() }}
                        {{ method_field('PUT') }}

                        <button type="submit" class="btn orange-button approve">Unarchive</button>
                    </form>
                </div>
